...
def _servo_child(cmd_q: MPQueue, resp_q: MPQueue, stop_evt: MPEvent) -> None:
    ...
    def _safe_enable(on: bool) -> bool:
        ...
        except (queue.Full, OSError, RuntimeError, ValueError, TypeError, EOFError):
            pass
        return False
    ...
    while not stop_evt.is_set():
        ...
        if now - last_hb >= HEARTBEAT_PERIOD_S:
            ...
            except (queue.Full, OSError, RuntimeError, ValueError, TypeError, EOFError): pass
        ...
        if servo_enabled and (now - last_apply) >= KEEPALIVE_PERIOD_S:
            try:
                ...
                _ka_fail = 0
            except (OSError, RuntimeError, TypeError, ValueError, AttributeError) as _e:
                _ka_fail += 1
                try:
        ...
        except (queue.Empty, OSError, RuntimeError, ValueError, EOFError):
            continue
...
class ServoManager:
    ...
    def stop(self) -> None:
        ...
        except (queue.Full, OSError, RuntimeError, ValueError, TypeError, EOFError, HTTPException): pass
        self._stop_evt.set()
        if self._proc:
            self._proc.join(timeout=1.5)
            if self._proc.is_alive():
                try: self._proc.terminate()
                except (OSError, RuntimeError, ValueError, AssertionError): pass
...
def _restart_camera() -> None:
    try:
        if _picam2:
            ...
            except (OSError, RuntimeError, TypeError, ValueError, AttributeError): pass
            try: _picam2.close()
            except (OSError, RuntimeError, TypeError, ValueError, AttributeError): pass
    except (OSError, RuntimeError, TypeError, ValueError, AttributeError):
        pass
