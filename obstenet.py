...
_IMPORT_DEP_ERRORS = (ImportError, OSError, RuntimeError)
_DIAGNOSTIC_HOOK_ERRORS = (OSError, RuntimeError, ValueError)
_THREAD_HOOK_ERRORS = (AttributeError, RuntimeError, TypeError)
_JSON_URL_ERRORS = (OSError, TimeoutError, UnicodeDecodeError, json.JSONDecodeError, TypeError, ValueError)
_SUBPROCESS_ERRORS = (OSError, subprocess.SubprocessError, TimeoutError, ValueError)
_HARDWARE_IO_ERRORS = (OSError, RuntimeError, AttributeError, TypeError, ValueError)
_QUEUE_ERRORS = (OSError, RuntimeError, TimeoutError, ValueError, TypeError, EOFError, queue.Empty, queue.Full)
...
def _import_dependencies(strict: bool = True) -> dict[str, str]:
    ...
    if Picamera2 is None or JpegEncoder is None or FileOutput is None or Transform is None:
        ...
        except _IMPORT_DEP_ERRORS as e:  # pragma: no cover - relies on platform packages
    ...
    if pantilthat is None:
        ...
        except _IMPORT_DEP_ERRORS as e:  # pragma: no cover - relies on hardware libs
...
except _DIAGNOSTIC_HOOK_ERRORS:
    pass
...
class VoltageMonitor:
    ...
    def _run(self) -> None:
        while not self._stop_evt.is_set():
            ...
            except (OSError, RuntimeError, TimeoutError, UnicodeDecodeError, json.JSONDecodeError, TypeError, ValueError):
                pass
    ...
    def _safe_enable(on: bool) -> bool:
        ...
        for _attempt in range(int(max(1, SERVO_RETRIES))):
            try:
                ...
                if _i2c_ping():
                    return True
            except _HARDWARE_IO_ERRORS as e:
                last_exc = e
            # brief backoff
            try:
                time.sleep(0.05)
    ...
    while not stop_evt.is_set():
        ...
        if servo_enabled and (now - last_apply) >= KEEPALIVE_PERIOD_S:
            try:
                ...
                _ka_fail = 0
            except _HARDWARE_IO_ERRORS as _e:
                _ka_fail += 1
                try:
...
class ServoManager:
    ...
    def stop(self) -> None:
        ...
        except (OSError, RuntimeError, TimeoutError, ValueError, TypeError, EOFError, queue.Full): pass
        self._stop_evt.set()
        if self._proc:
...
def _restart_camera() -> None:
    try:
        if _picam2:
            ...
            except _HARDWARE_IO_ERRORS: pass
            try: _picam2.close()
            except _HARDWARE_IO_ERRORS: pass
    except _HARDWARE_IO_ERRORS:
        pass
